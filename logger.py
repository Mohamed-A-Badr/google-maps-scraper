...
country_log_file = "country_boundary_exception.log"
...
if not os.path.exists(country_log_file):
    with open(country_log_file, "w") as f:
        f.write("")
...
file_handler = logging.FileHandler(filename=country_log_file, encoding="utf-8")
formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
file_handler.setFormatter(formatter)
